...
class Service_Factory:
    ...
    def auth(self):
        user_db = self.db_registry.get('user')
        auth_service = Auth_Service(user_db)
        return auth_service

    def comment(self):
        comment_db = self.db_registry.get("comment")
        vote_db = self.db_registry.get("vote")

        comment_service = Comment_Service(comment_db, vote_db)
        return comment_service

    def job_post(self):
        job_post_db = self.db_registry.get("job_post")

        job_post_service = Job_Post_Service(job_post_db)
        return job_post_service

    def rating(self):
        rating_db = self.db_registry.get("rating")
        job_post_db = self.db_registry.get("job_post")
    ...
    def request(self):
        request_db = self.db_registry.get("request")

        request_service = Request_Service(request_db)
        return request_service

    def unban_request(self):
        unban_request_db = self.db_registry.get("unban_request")
